...
class FaceSamplesLoaderSubprocessor(Subprocessor):
    ...
    def on_result (self, host_dict, data, result):
        idx, alphaimg = result
        self.result[idx] = (self.image_paths[idx], alphaimg)
    ...
    class Cli(Subprocessor.Cli):
        ...
        def process_data(self, data):
            idx, filename = data
            alphaimg = ALPHAIMG.load (Path(filename))

            if alphaimg is None or not alphaimg.has_data():
                self.log_err (f"FaceSamplesLoader: {filename} is not a alpha image file.")
                data = None
            else:
                data = (alphaimg.get_face_type(),
                        alphaimg.get_shape(),
                        alphaimg.get_landmarks(),
                        alphaimg.get_seg_ie_polys(),
                        alphaimg.get_xseg_mask_compressed(),
                        alphaimg.get_eyebrows_expand_mod(),
                        alphaimg.get_source_filename() )
