...
class ALPHAJPG(object):
    ...
    def load(filename, loader_func=None):
        try:
            inst = ALPHAJPG.load_raw (filename, loader_func=loader_func)
            inst.alpha_dict = {}
            ...
            for chunk in inst.chunks:
                if chunk['name'] == 'APP0':
                    ...
                    if type(chunk['data']) == bytes:
                        inst.alpha_dict = pickle.loads(chunk['data'])

            return inst
        except Exception as e:
            io.log_err (f'Exception occured while ALPHAJPG.load : {traceback.format_exc()}')
            return None

    def has_data(self):
        return len(self.alpha_dict.keys()) != 0
    ...
    def dump(self):
        ...
        alphachunk = {'name' : 'APP15',
                    'm_h' : 0xEF,
                    'data' : pickle.dumps(dict_data),
                    'ex_data' : None,
                    }
        self.chunks.insert (last_app_chunk+1, alphachunk)
    ...
    def set_dict (self, dict_data=None):
        self.alpha_dict = dict_data

    def get_face_type(self):            return self.alpha_dict.get('face_type', FaceType.toString (FaceType.FULL) )
    def set_face_type(self, face_type): self.alpha_dict['face_type'] = face_type

    def get_landmarks(self):            return np.array ( self.alpha_dict['landmarks'] )
    def set_landmarks(self, landmarks): self.alpha_dict['landmarks'] = landmarks

    def get_eyebrows_expand_mod(self):                      return self.alpha_dict.get ('eyebrows_expand_mod', 1.0)
    def set_eyebrows_expand_mod(self, eyebrows_expand_mod): self.alpha_dict['eyebrows_expand_mod'] = eyebrows_expand_mod

    def get_source_filename(self):                  return self.alpha_dict.get ('source_filename', None)
    def set_source_filename(self, source_filename): self.alpha_dict['source_filename'] = source_filename

    def get_source_rect(self):              return self.alpha_dict.get ('source_rect', None)
    def set_source_rect(self, source_rect): self.alpha_dict['source_rect'] = source_rect

    def get_source_landmarks(self):                     return np.array ( self.alpha_dict.get('source_landmarks', None) )
    def set_source_landmarks(self, source_landmarks):   self.alpha_dict['source_landmarks'] = source_landmarks

    def get_image_to_face_mat(self):
        mat = self.alpha_dict.get ('image_to_face_mat', None)
        if mat is not None:
            return np.array (mat)
        return None
    def set_image_to_face_mat(self, image_to_face_mat):   self.alpha_dict['image_to_face_mat'] = image_to_face_mat

    def has_seg_ie_polys(self):
        return self.alpha_dict.get('seg_ie_polys',None) is not None

    def get_seg_ie_polys(self):
        d = self.alpha_dict.get('seg_ie_polys',None)
        if d is not None:
            d = SegIEPolys.load(d)
        else:
            d = SegIEPolys()
    ...
    def set_seg_ie_polys(self, seg_ie_polys):
        ...
        self.alpha_dict['seg_ie_polys'] = seg_ie_polys

    def has_xseg_mask(self):
        return self.alpha_dict.get('xseg_mask',None) is not None

    def get_xseg_mask_compressed(self):
        mask_buf = self.alpha_dict.get('xseg_mask',None)
        if mask_buf is None:
            return None
    ...
    def set_xseg_mask(self, mask_a):
        if mask_a is None:
            self.alpha_dict['xseg_mask'] = None
            return
