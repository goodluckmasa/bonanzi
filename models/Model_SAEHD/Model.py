...
class SAEHDModel(ModelBase):
    ...
    def on_initialize_options(self):
        ...
        if self.is_first_run() or ask_override:
            if (self.read_from_conf and not self.config_file_exists) or not self.read_from_conf:
                ...
                self.options['pretrain'] = io.input_bool ("Enable pretraining mode", default_pretrain, help_message="Pretrain the model with large amount of various faces. After that, model can be used to train the fewiufs more quickly. Forces random_warp=N, random_flips=Y, gan_power=0.0, lr_dropout=N, styles=0.0, uniform_yaw=Y")
    ...
    def on_initialize(self):
        ...
        model_archi = nn.BetaArchi(resolution, use_fp16=use_fp16, opts=archi_opts)
