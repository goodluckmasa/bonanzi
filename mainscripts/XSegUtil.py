...
def apply_xseg(input_path, model_path):
    ...
    for filepath in io.progress_bar_generator(images_paths, "Processing"):
        ...
        if alphaimg is None or not alphaimg.has_data():
            io.log_info(f'{filepath} is not a ALPHAIMG')
            continue
        
        img = cv2_imread(filepath).astype(np.float32) / 255.0
        h,w,c = img.shape
        
        img_face_type = FaceType.fromString( alphaimg.get_face_type() )
        if face_type is not None and img_face_type != face_type or img_face_type == FaceType.CUSTOM: # custom always goes for eqvivalents
            lmrks = alphaimg.get_source_landmarks()
...
def fetch_xseg(input_path):
    ...
    for filepath in io.progress_bar_generator(images_paths, "Processing"):
        ...
        ie_polys = alphaimg.get_seg_ie_polys()
...
def remove_xseg(input_path):
    ...
    for filepath in io.progress_bar_generator(images_paths, "Processing"):
        ...
        if alphaimg.has_xseg_mask():
            alphaimg.set_xseg_mask(None)
            alphaimg.save()
            files_processed += 1
...
def remove_xseg_labels(input_path):
    ...
    for filepath in io.progress_bar_generator(images_paths, "Processing"):
        ...
        if alphaimg.has_seg_ie_polys():
            alphaimg.set_seg_ie_polys(None)
            alphaimg.save()            
            files_processed += 1
