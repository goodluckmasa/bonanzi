...
class FacesetResizerSubprocessor(Subprocessor):
    ...
    class Cli(Subprocessor.Cli):
        ...
        def process_data(self, filepath):
            try:
                alphaimg = ALPHAIMG.load (filepath)
                if alphaimg is None or not alphaimg.has_data():
                    self.log_err (f"{filepath.name} is not a alpha image file")
                else:
                    ...
                    if face_type is not None:
                        ...
                        alpha_dict = alphaimg.get_dict()
                        alphaimg = ALPHAIMG.load (output_filepath)
                        alphaimg.set_dict(alpha_dict)
                        
                        xseg_mask = alphaimg.get_xseg_mask()
                        if xseg_mask is not None:
                            ...
                            alphaimg.set_xseg_mask(xseg_mask)
                        
                        seg_ie_polys = alphaimg.get_seg_ie_polys()
                        ...
                        alphaimg.set_seg_ie_polys(seg_ie_polys)
                        
                        lmrks = LandmarksProcessor.transform_points(lmrks, mat)
                        alphaimg.set_landmarks(lmrks)
    
                        image_to_face_mat = alphaimg.get_image_to_face_mat()
                        if image_to_face_mat is not None:
                            image_to_face_mat = LandmarksProcessor.get_transform_mat ( alphaimg.get_source_landmarks(), image_size, face_type )
                            alphaimg.set_image_to_face_mat(image_to_face_mat)
                        alphaimg.set_face_type( FaceType.toString(face_type) )
                        alphaimg.save()
                        ...
                        if image_to_face_mat is not None:
                            face_type = FaceType.fromString ( alphaimg.get_face_type() )
                            image_to_face_mat = LandmarksProcessor.get_transform_mat ( alphaimg.get_source_landmarks(), image_size, face_type )
                            alphaimg.set_image_to_face_mat(image_to_face_mat)
                        alphaimg.save()
