        ...
        if cfg.type == MergerConfig.TYPE_MASKED:
            ...
            if packed_samples is not None:
                ...
                def generator():
                    for sample in io.progress_bar_generator( packed_samples, "Collecting alignments"):
                        ...
                        yield filepath, ALPHAIMG.load(filepath, loader_func=lambda x: sample.read_raw_file()  )
            else:
                def generator():
                    for filepath in io.progress_bar_generator( pathex.get_image_paths(aligned_path), "Collecting alignments"):
                        filepath = Path(filepath)
                        yield filepath, ALPHAIMG.load(filepath)

            alignments = {}
            multiple_faces_detected = False

            for filepath, alphaimg in generator():
                if alphaimg is None or not alphaimg.has_data():
                    io.log_err (f"{filepath.name} is not a alpha image file")
                    continue

                source_filename = alphaimg.get_source_filename()
                ...
                alignments_ar.append ( (alphaimg.get_source_landmarks(), filepath, source_filepath, alphaimg ) )
            ...
            for p in input_path_image_paths:
                ...
                if data == None:
                    ...
                    alpha_images_list = [d[1] for d in data]
                    frame_info=FrameInfo(filepath=cur_path, landmarks_list=landmarks_list, alpha_images_list=alpha_images_list)
                    frame = InteractiveMergerSubprocessor.Frame(frame_info=frame_info)
...
for filepath in io.progress_bar_generator(input_path_image_paths, "Collecting info"):
    ...
    alphaimg = ALPHAIMG.x(filepath)
    if alphaimg is None:
        io.log_err ("%s is not a alpha image file" % (filepath.name) )
        continue
    filesdata += [ ( FrameInfo(filepath=filepath, landmarks_list=[alphaimg.get_landmarks()] ), alphaimg.get_source_filename() ) ]
