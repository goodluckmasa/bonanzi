    ...
    class Cli(Subprocessor.Cli):
        ...
        def final_stage(data,
            ...
            for rect, image_landmarks in zip( rects, landmarks ):
                ...
                alphaimg = ALPHAJPG.load(output_filepath)
                alphaimg.set_face_type(FaceType.toString(face_type))
                alphaimg.set_landmarks(face_image_landmarks.tolist())
                alphaimg.set_source_filename(filepath.name)
                alphaimg.set_source_rect(rect)
                alphaimg.set_source_landmarks(image_landmarks.tolist())
...
def main(detector=None,
    ...
    if face_type is None:
        if manual_output_debug_fix:
            ...
            if len(files) != 0:
                alphaimg = ALPHAIMG.load(Path(files[0]))
