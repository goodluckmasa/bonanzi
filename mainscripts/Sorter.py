    ...
    class Cli(Subprocessor.Cli):
        ...
        def process_data(self, data):
            ...
            if alphaimg is None or not alphaimg.has_data():
                ...
                face_mask = LandmarksProcessor.get_image_hull_mask (image.shape, alphaimg.get_landmarks())
...
def sort_by_face_yaw(input_path):
    ...
    for filepath in io.progress_bar_generator( pathex.get_image_paths(input_path), "Loading"):
        ...
        pitch, yaw, roll = LandmarksProcessor.estimate_pitch_yaw_roll ( alphaimg.get_landmarks(), size=alphaimg.get_shape()[1] )
...
def sort_by_face_source_rect_size(input_path):
    ...
    for filepath in io.progress_bar_generator( pathex.get_image_paths(input_path), "Loading"):
        ...
        source_rect = alphaimg.get_source_rect()
...
def sort_by_hist_dissim(input_path):
    ...
    for filepath in io.progress_bar_generator( pathex.get_image_paths(input_path), "Loading"):
        ...
        if alphaimg is not None and alphaimg.has_data():
            face_mask = LandmarksProcessor.get_image_hull_mask (image.shape, alphaimg.get_landmarks())
            image = (image*face_mask).astype(np.uint8)
...
def sort_by_origname(input_path):
    ...
    for filepath in io.progress_bar_generator( pathex.get_image_paths(input_path), "Loading"):
        ...
        img_list.append( [str(filepath), alphaimg.get_source_filename()] )
        ...
        def process_data(self, data):
            ...
            try:
                ...
                if self.faster:
                    ...
                    face_mask = LandmarksProcessor.get_image_hull_mask (gray.shape, alphaimg.get_landmarks())     
